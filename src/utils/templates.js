...
export const TEMPLATES = [
  ...
  },
  {
    ...
    profile: () => ({
      ...
      widgets: [
        ...
        w('badges', {
          data: { badges: [{ emoji: '◼', label: 'RAW' }, { emoji: '◻', label: 'STARK' }] },
          style: { borderRadius: 0, fontFamily: 'JetBrains Mono' },
        }),
        w('socials', { style: { borderRadius: 0, bgOpacity: 0 } }),
        w('games', { style: { borderRadius: 0 } }),
  ...
  },
  {
    ...
    profile: () => ({
      ...
      widgets: [
        ...
        w('badges', {
          data: { badges: [{ emoji: '○', label: 'Open to work' }] },
          style: { bgOpacity: 0 },
        }),
        w('socials', { style: { bgOpacity: 0 } }),
        w('clock', { data: { format24h: true, showSeconds: false }, style: { bgOpacity: 0 } }),
  ...
  },
  {
    ...
    profile: () => ({
      ...
      theme: {
        ...
        cursorTrail: 'glow',
        particles: 'snow',
        splash: { enabled: true, text: 'welcome', subtitle: 'take a moment' },
        entryAnimation: 'blur',
        customCss: '',
      },
      background: { type: 'none', image: '', video: '', youtubeId: '', overlayColor: '#000', overlayOpacity: 0.1 },
      music: { enabled: false, src: '', autoplay: false, volume: 0.35, trackTitle: '', artist: '' },
      widgets: [
        w('avatar', { data: { username: 'Aurora', bio: 'Designer · cold takes only.', textEffect: 'gradient' } }),
        w('badges', {
          data: { badges: [{ emoji: '❄', label: 'Calm' }, { emoji: '✦', label: 'Crisp' }] },
        }),
        w('socials'),
        w('weather', { data: { city: 'Reykjavik' } }),
        w('clock', { data: { format24h: true, showSeconds: false } }),
        w('nowPlaying'),
      ],
    }),
  },
  {
    ...
    profile: () => ({
      ...
      background: { type: 'none', image: '', video: '', youtubeId: '', overlayColor: '#1a1209', overlayOpacity: 0.4 },
      music: { enabled: false, src: '', autoplay: true, volume: 0.3, trackTitle: '', artist: '' },
      widgets: [
        w('avatar', { data: { username: 'beans', bio: 'beats · books · brewed slow.', textEffect: 'glow' } }),
        w('badges', {
          data: { badges: [{ emoji: '☕', label: 'Caffeinated' }, { emoji: '🎧', label: 'Lo-fi' }, { emoji: '📚', label: 'Reader' }] },
        }),
        w('socials'),
        w('nowPlaying'),
        w('musicProgress'),
        w('clock', { data: { format24h: false } }),
      ],
    }),
  },
  {
    ...
    profile: () => ({
      ...
      theme: {
        ...
        cursorTrail: 'glow',
        particles: 'snow',
        splash: { enabled: true, text: '桜', subtitle: 'cherry blossoms' },
        entryAnimation: 'fade',
        customCss: '',
      },
      background: { type: 'none', image: '', video: '', youtubeId: '', overlayColor: '#0c0509', overlayOpacity: 0.2 },
      music: { enabled: false, src: '', autoplay: false, volume: 0.35, trackTitle: '', artist: '' },
      widgets: [
        w('avatar', { data: { username: 'hanami', bio: 'one petal at a time.', textEffect: 'gradient' } }),
        w('badges', {
          data: { badges: [{ emoji: '🌸', label: 'Bloom' }, { emoji: '🍃', label: 'Quiet' }] },
        }),
        w('socials'),
        w('qa'),
        w('clock', { data: { format24h: true } }),
      ],
    }),
  },
  {
    ...
    profile: () => ({
      ...
      theme: {
        ...
        cursorTrail: 'neon',
        particles: 'stars',
        splash: { enabled: true, text: 'GAME ON', subtitle: '★ insert coin ★' },
        entryAnimation: 'scale',
        customCss: '',
      },
      background: { type: 'none', image: '', video: '', youtubeId: '', overlayColor: '#0a0220', overlayOpacity: 0.3 },
      music: { enabled: false, src: '', autoplay: false, volume: 0.4, trackTitle: '', artist: '' },
      widgets: [
        w('avatar', { data: { username: 'NIGHT-DRIVER', bio: 'sunset on the 405.', textEffect: 'neon' } }),
        w('badges', {
          data: { badges: [{ emoji: '🏁', label: 'Outrun' }, { emoji: '🌴', label: 'Miami' }, { emoji: '💾', label: '198X' }] },
        }),
        w('socials'),
        w('games'),
        w('nowPlaying'),
        w('musicProgress'),
        w('clock', { data: { format24h: false, showSeconds: true } }),
      ],
    }),
  },
];
